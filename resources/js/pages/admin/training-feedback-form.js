...
export default defineComponent({
    name: 'AdminTrainingFeedbackForm',
    components: { AdminShell, AttachmentCard, ConfirmActionModal, Head, Link },
    props: {
        serviceRequest: { type: Object, required: true },
        trainingRequest: { type: Object, required: true },
        logs: { type: Array, default: () => [] },
        feedbackLinks: { type: Array, default: () => [] },
    },
    setup(props) {
        const activeTab = useQueryTab(['service-request', 'service-fee', 'verify-payment', 'feedback', 'logs'], 'service-request');
        ...
        const sendReminder = () => {
            ...
            sending.value = true;
            router.post(`/admin/requests/${props.serviceRequest.id}/training-request/feedback/remind`, {}, {
                preserveScroll: true,
                onFinish: () => {
                    sending.value = false;
                },
        ...
        const generateLink = () => {
            ...
            router.post(`/admin/requests/${props.serviceRequest.id}/training-request/feedback/generate-link`, {}, {
                preserveScroll: true,
                onFinish: () => {
            ...
            <section class="border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
                <div class="flex flex-wrap items-start justify-between gap-4">
                    <div>
                        ...
                        <div class="mt-1 flex flex-wrap items-center gap-3">
                            <h2 class="text-2xl font-bold text-slate-900">Service request #{{ serviceRequest.id }} &middot; {{ trainingRequest.fee.reference_no }}</h2>
                        ...
                        <p class="mt-1 text-slate-600">{{ isCompleted ? 'View the request, service fee, payment, and client feedback.' : 'Review the request, service fee, payment, and client feedback.' }}</p>
                    </div>
                ...
                <template v-else-if="activeTab === 'verify-payment'">
                    <div class="mt-7 w-full">
                        ...
                        <p class="mt-1 text-sm text-slate-500">Payment for {{ trainingRequest.fee.reference_no }} has already been verified.</p>

                        <dl class="mt-5 grid gap-5 sm:grid-cols-2">
                            <div>
                                <dt class="text-xs font-bold uppercase tracking-wide text-slate-400">Bill number</dt>
                                <dd class="mt-1 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-slate-700">{{ trainingRequest.fee.bill_no }}</dd>
                                <dd class="mt-2"><AttachmentCard :href="trainingRequest.fee.bill_attachment_url" name="Bill attachment" /></dd>
                            </div>
                            <div>
                                <dt class="text-xs font-bold uppercase tracking-wide text-slate-400">O.R. number</dt>
                                <dd class="mt-1 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-slate-700">{{ trainingRequest.fee.or_no }}</dd>
                                <dd class="mt-2"><AttachmentCard :href="trainingRequest.fee.or_attachment_url" name="OR attachment" /></dd>
                            </div>
                        </dl>
                    </div>
