...
export default defineComponent({
    ...
    setup(props) {
        ...
        const sendReminder = () => {
            ...
            sending.value = true;
            router.post(`/admin/requests/${props.serviceRequest.id}/processing-request/feedback/remind`, {}, {
                preserveScroll: true,
                onFinish: () => {
                    sending.value = false;
                },
        ...
        const generateLink = () => {
            ...
            router.post(`/admin/requests/${props.serviceRequest.id}/processing-request/feedback/generate-link`, {}, {
                preserveScroll: true,
                onFinish: () => {
            ...
            <section class="border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
                <div class="flex flex-wrap items-start justify-between gap-4">
                    <div>
                        ...
                        <div class="mt-1 flex flex-wrap items-center gap-3">
                            <h2 class="text-2xl font-bold text-slate-900">Service request #{{ serviceRequest.id }} &middot; {{ processingRequest.reference_no }}</h2>
                ...
                <template v-if="activeTab === 'service-request'">
                    <div class="mt-7 space-y-8">
                        <div class="flex justify-end">
                            <a :href="'/admin/requests/' + serviceRequest.id + '/processing-request/pdf'" class="inline-flex items-center gap-2 rounded-lg border border-slate-300 px-4 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50">
                        ...
                        <section class="border-t border-slate-100 pt-7">
                            ...
                            <dl class="mt-4 grid gap-4 sm:grid-cols-2">
                                <div>
                                    ...
                                    <dd class="mt-1 font-semibold text-slate-900">{{ processingRequest.reference_no }}</dd>
                                </div>
                                <div>
                                    <dt class="text-xs font-bold uppercase tracking-wide text-slate-400">PTRI sample number</dt>
                                    <dd class="mt-1 font-semibold text-slate-900">{{ processingRequest.sample_no }}</dd>
                                </div>
                                <div>
                                    <dt class="text-xs font-bold uppercase tracking-wide text-slate-400">Due date</dt>
                                    <dd class="mt-1 text-slate-700">{{ processingRequest.due_date }}</dd>
                                </div>
                                <div>
                                    <dt class="text-xs font-bold uppercase tracking-wide text-slate-400">No./Type of sample</dt>
                                    <dd class="mt-1 text-slate-700">{{ processingRequest.sample_type }}</dd>
                                </div>
                            </dl>
                            <div class="mt-6 overflow-x-auto">
                                <table class="w-full min-w-[500px] text-left text-sm">
                                    <thead class="border-y border-slate-200 text-xs uppercase tracking-wide text-slate-500">
                                        <tr>
                                            <th class="px-3 py-2">Service request</th>
                                    ...
                                    <tbody>
                                        <tr v-for="(item, index) in processingRequest.items" :key="index" class="border-b border-slate-100">
                            ...
                            <dl class="mt-6 ml-auto max-w-sm space-y-2 text-sm">
                                <div class="flex justify-between"><dt class="text-slate-500">Sub total</dt><dd class="font-semibold text-slate-900">{{ peso(processingRequest.sub_total) }}</dd></div>
                                <div class="flex justify-between"><dt class="text-slate-500">Discount</dt><dd class="font-semibold text-slate-900">-{{ peso(processingRequest.discount) }}</dd></div>
                                <div class="flex justify-between border-t border-slate-200 pt-2 text-base"><dt class="font-bold text-slate-900">Total fee</dt><dd class="font-bold text-slate-900">{{ peso(processingRequest.total_fee) }}</dd></div>
                            </dl>
                        </section>
                    </div>
                ...
                <template v-else-if="activeTab === 'verify-payment'">
                    <div class="mt-7 w-full">
                        ...
                        <p class="mt-1 text-sm text-slate-500">Payment for {{ processingRequest.reference_no }} has already been verified.</p>

                        <dl class="mt-5 grid gap-5 sm:grid-cols-2">
                            <div>
                                <dt class="text-xs font-bold uppercase tracking-wide text-slate-400">OP number</dt>
                                <dd class="mt-1 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-slate-700">{{ processingRequest.op_no }}</dd><dd class="mt-2"><AttachmentCard :href="processingRequest.op_attachment_url" name="OP attachment" /></dd>
                            </div>
                            <div>
                                <dt class="text-xs font-bold uppercase tracking-wide text-slate-400">OR number</dt>
                                <dd class="mt-1 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-slate-700">{{ processingRequest.or_no }}</dd><dd class="mt-2"><AttachmentCard :href="processingRequest.or_attachment_url" name="OR attachment" /></dd>
                            </div>
                        </dl>
                    </div>
