...
export default defineComponent({
    name: 'AdminProcessingPaymentForm',
    components: { AdminShell, ConfirmActionModal, CodeConfirmationModal, Head, Link },
    props: { serviceRequest: { type: Object, required: true }, processingRequest: { type: Object, required: true } },
    setup(props) {
        const activeTab = useQueryTab(['service-request', 'payment-verification'], 'service-request');
        const form = reactive({
            op_no: props.processingRequest.op_no || '',
            or_no: props.processingRequest.or_no || '',
            op_attachment: null,
            or_attachment: null,
        });
        const errors = ref({});
        ...
        const submit = (code) => {
            ...
            router.post(`/admin/requests/${props.serviceRequest.id}/processing-request/payment`, {
                op_no: form.op_no,
                or_no: form.or_no,
        ...
        const sendReminder = () => {
            ...
            router.post(`/admin/requests/${props.serviceRequest.id}/processing-request/payment/remind`, {}, {
                preserveScroll: true,
                onFinish: () => {
    ...
    template: `
        ...
        <AdminShell active="requests" title="Verify Payment">
            <section class="border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
                <div class="flex flex-wrap items-start justify-between gap-4">
                    <div>
                        ...
                        <h2 class="mt-1 text-2xl font-bold text-slate-900">Service request #{{ serviceRequest.id }} &middot; {{ processingRequest.reference_no }}</h2>
                    ...
                    <div class="mt-7">
                        <div class="flex flex-wrap items-start justify-between gap-4">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-slate-500">Record the official payment references for {{ processingRequest.reference_no }}.</p>
                            </div>
            ...
            <CodeConfirmationModal
                ...
                :message="'Confirm OP ' + form.op_no + ' and OR ' + form.or_no + ' for ' + processingRequest.reference_no + '. Enter the four-digit code below to confirm.'"
