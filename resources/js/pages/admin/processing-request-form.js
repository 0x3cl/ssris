...
const blankItem = () => ({ item: '', weight: '', quantity: 1, unit_fee: 0 });
...
export default defineComponent({
    name: 'AdminProcessingRequestForm',
    ...
    setup(props) {
        ...
        const validate = () => {
            ...
            if (!form.sample_type.trim()) {
                newErrors.sample_type = 'Enter the number/type of sample.';
            }
            ...
            form.items.forEach((item, index) => {
                ...
                if (!item.weight.trim()) newErrors[`items.${index}.weight`] = 'Enter the weight.';
        ...
        const submit = () => {
            ...
            router.post(`/admin/requests/${props.serviceRequest.id}/processing-request`, {
    ...
    template: `
        <Head title="Processing services request form" />
        <AdminShell active="requests" title="Processing Services Request Form">
            <section class="border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
                ...
                <template v-if="currentStep === 1">
                    ...
                    <form class="mt-8 border-t border-slate-200 pt-8" @submit.prevent="goToReview">
                        ...
                        <p class="mt-1 text-sm text-slate-500">The customer reference number and PTRI sample number are assigned automatically when you submit.</p>
                        <div class="mt-5 grid gap-5 md:grid-cols-6">
                            <label class="md:col-span-2">
                                <span class="text-sm font-medium text-slate-700">Customer reference number</span>
                                <input :value="'PRC-' + clientInitials + '-##### '" disabled tabindex="-1" class="mt-1 w-full cursor-not-allowed rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-slate-700" />
                            </label>
                            <label class="md:col-span-2">
                                <span class="text-sm font-medium text-slate-700">PTRI sample number</span>
                                <input :value="'#####-' + clientInitials + ' '" disabled tabindex="-1" class="mt-1 w-full cursor-not-allowed rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-slate-700" />
                            </label>
                            <label class="md:col-span-2">
                            ...
                            <label class="md:col-span-6">
                                <span class="text-sm font-medium text-slate-700">No./Type of sample</span>
                                <input v-model="form.sample_type" class="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2.5" />
                            ...
                            <table class="w-full min-w-[800px] text-left">
                                ...
                                <tbody>
                                    <tr v-for="(item, index) in form.items" :key="index" class="border-b border-slate-100 align-top">
                                        ...
                                        <td class="px-3 py-3">
                                            <input v-model="item.weight" class="w-32 rounded-lg border border-slate-300 px-3 py-2" />
                                            <p v-if="errors['items.' + index + '.weight']" class="mt-1 text-xs text-rose-600">{{ errors['items.' + index + '.weight'] }}</p>
                                        </td>
                                        <td class="px-3 py-3">
                        ...
                        <section class="border-t border-slate-100 pt-7">
                            ...
                            <dl class="mt-4 grid gap-4 sm:grid-cols-2">
                                <div>
                                    ...
                                    <dd class="mt-1 font-semibold text-slate-900">PRC-{{ clientInitials }}-##### <span class="font-normal text-slate-500"></span></dd>
                                </div>
                                <div>
                                    <dt class="text-xs font-bold uppercase tracking-wide text-slate-400">PTRI sample number</dt>
                                    <dd class="mt-1 font-semibold text-slate-900">#####-{{ clientInitials }} <span class="font-normal text-slate-500"></span></dd>
                                </div>
                                <div>
            ...
            <ConfirmActionModal
                ...
                title="Submit this processing services request form?"
