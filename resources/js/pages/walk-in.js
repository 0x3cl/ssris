...
export default defineComponent({
    ...
    setup(props) {
        ...
        const errorLabels = {
            ...
            visit_date: 'Select desired date of tour', visit_time: 'Select desired time of tour', message: 'Message to us', no_persons: 'Number of persons', no_groups: 'Number of group(s)/batch(es)', technology_assistance: 'Technology assistance', visit_objectives: 'Objective of this visit',
        };
        ...
        const markVisitRequestFields = () => nextTick(() => markRequiredFields('#visit-request', ['Select desired date of tour', 'Select desired time of tour', 'Message to us', 'Number of persons', 'Number of group(s)/batch(es)', 'Technology assistance', 'Objective of this visit']));
        ...
        const tourFieldNames = new Set(['visit_date', 'visit_time', 'message', 'no_persons', 'no_groups', 'technology_assistance', 'visit_objectives', 'testing_lab', 'pilot_plant', 'others', 'facilities']);

        const routeToStepForErrors = (errors) => {
            const hasClientError = Object.keys(errors).some((field) => !tourFieldNames.has(field.split('.')[0]));

            if (hasClientError) return 2;

            return isTourService.value ? visitRequestStep : 2;
        };
        ...
        const backFromReview = () => {
            currentStep.value = isTourService.value ? visitRequestStep : 2;
        ...
        <main class="min-h-screen bg-white px-4 py-8 sm:px-6 sm:py-12">
            <section class="mx-auto w-full max-w-6xl">
                ...
                <div class="mx-auto mt-10 w-full py-5 sm:py-8">
                    <WalkInStepper :current-step="currentStep" :steps="stepLabels" />
                    ...
                    <section v-else-if="currentStep === 2" class="mt-6 rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200 sm:p-8">
                        ...
                        <form v-if="form.email" id="client-details" class="mt-8 border-t border-slate-200 pt-8" @submit.prevent="advanceFromClientDetails">
                            ...
                            <div class="mt-8 flex items-center justify-between gap-3">
                                <button type="button" class="inline-flex items-center gap-2 rounded-xl border border-slate-300 px-5 py-3 text-sm font-bold uppercase tracking-wide text-[#07559e] transition hover:border-[#07559e] hover:bg-sky-50" @click="currentStep = 1">
                    ...
                    <section v-else-if="currentStep === visitRequestStep && isTourService" class="mt-6 rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200 sm:p-8">
                        ...
                        <form id="visit-request" class="mt-8 space-y-6" @submit.prevent="validateDetails">
                            <div class="grid gap-5 sm:grid-cols-2">
                                <label>
                                    <span class="text-sm font-medium text-slate-700">Select desired date of tour</span>
                                    <input v-model="form.visit_date" type="date" :min="minVisitDate" class="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2.5" />
                                </label>
                                <label>
                                    <span class="text-sm font-medium text-slate-700">Select desired time of tour</span>
                                    <input v-model="form.visit_time" type="time" class="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2.5" />
                                </label>
                            </div>
                            <label class="block">
                    ...
                    <section v-else-if="currentStep === reviewStep" class="mt-6 rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200 sm:p-8">
                        <div class="border-b border-slate-200 pb-6">
                            ...
                            <p class="mt-1 text-slate-600">Check the information below before submitting your walk-in request.</p>
                        </div>
                        <form class="mt-8 space-y-8" @submit.prevent="openTermsModal">
                            ...
                            <section v-if="isTourService" class="border-t border-slate-100 pt-7">
                                ...
                                <div class="mt-4 grid gap-5 md:grid-cols-6">
                                    <label class="md:col-span-3">
                                        <span class="text-sm font-medium text-slate-700">Desired date of tour</span>
                                        <input :value="formatDate(form.visit_date)" readonly class="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5" />
                                    </label>
                                    <label class="md:col-span-3">
                                        <span class="text-sm font-medium text-slate-700">Desired time of tour</span>
                                        <input :value="formatTime(form.visit_time)" readonly class="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5" />
                                    </label>
                                    <label class="md:col-span-6">
                                        <span class="text-sm font-medium text-slate-700">Message to us</span>
                            ...
                            <div class="flex justify-between gap-3 border-t border-slate-100 pt-7">
                                ...
                                <button type="submit" class="inline-flex items-center gap-2 rounded-xl bg-[#00aeef] px-5 py-3 text-sm font-bold uppercase tracking-wide text-white transition hover:bg-[#008dcc]">
                                    Submit walk-in request<i class="fa-solid fa-arrow-right" aria-hidden="true"></i>
                                </button>
                            </div>
                        </form>
                    </section>
