...
use App\Services\RddPdfService;
...
class RddRequestController extends Controller
{
    ...
    public function store(StoreRddRequestRequest $request, ServiceRequest $serviceRequest): RedirectResponse
    {
        ...
        $this->serviceRequestLogger->log(
            ...
            'R&D request form saved and the request moved to for payment.',
        );

        return to_route('admin.requests.rdd.payment.edit', ['serviceRequest' => $serviceRequest, 'tab' => 'payment-verification'])
    ...
    public function downloadPdf(ServiceRequest $serviceRequest): HttpResponse|RedirectResponse
    {
        ...
        $document = app(RddPdfService::class)->render(
        ...
        $filename = "{$rddRequest->reference_no}.pdf";
    ...
    public function updatePayment(Request $request, ServiceRequest $serviceRequest): RedirectResponse
    {
        ...
        return to_route('admin.requests.rdd.feedback.edit', ['serviceRequest' => $serviceRequest, 'tab' => 'feedback'])
    ...
    public function downloadPaymentAttachment(ServiceRequest $serviceRequest, string $type): BinaryFileResponse|StreamedResponse
    {
        ...
        $path = $serviceRequest->rddRequest?->getAttribute($type.'_attachment');
    ...
    public function sendPaymentReminder(ServiceRequest $serviceRequest): RedirectResponse
    {
        ...
        $serviceRequest->load(['client', 'rddRequest']);
        ...
        $error = $this->formTemplateMailer->send(FormTemplateKey::PaymentReminder, $serviceRequest->client->email, [
            ...
            'reference_no' => $serviceRequest->rddRequest->reference_no,
            'amount' => 'PHP '.number_format((float) $serviceRequest->rddRequest->total_fee, 2),
            'due' => $serviceRequest->rddRequest->due_date->format('F j, Y'),
        ], $serviceRequest);
    ...
    public function editFeedback(ServiceRequest $serviceRequest): Response|RedirectResponse
    {
        ...
        if (! $serviceRequest->rddRequest()->exists()) {
            return to_route('admin.requests.index')->with('error', 'The R&D request form is not available.');
        }

        $serviceRequest->load(['client', 'rddRequest.items', 'logs', 'feedbackLinks']);
    ...
    public function sendFeedbackReminder(ServiceRequest $serviceRequest): RedirectResponse
    {
        ...
        $error = $this->formTemplateMailer->send(FormTemplateKey::FeedbackReminder, $serviceRequest->client->email, [
            'reference' => $serviceRequest->rddRequest->reference_no,
            'due_date' => $serviceRequest->rddRequest->due_date->format('F j, Y'),
        ...
        return $serviceRequest->feedbackLinks
            ...
            ->map(fn (FeedbackLink $link): array => [
                ...
                'response_url' => $link->isSubmitted()
                    ? "/admin/requests/{$serviceRequest->id}/rdd-request/feedback/responses/{$link->id}"
                    : null,
            ])
            ->values()
            ->all();
    ...
    private function rddRequestPayload(RddRequest $rddRequest): array
    {
        return [
            ...
            'op_attachment_url' => $rddRequest->op_attachment ? route('admin.requests.rdd.attachment', ['serviceRequest' => $rddRequest->service_request_id, 'type' => 'op']) : null,
            'or_attachment_url' => $rddRequest->or_attachment ? route('admin.requests.rdd.attachment', ['serviceRequest' => $rddRequest->service_request_id, 'type' => 'or']) : null,
